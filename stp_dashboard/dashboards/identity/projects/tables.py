...
#        super(TenantsTable, self).__init__(request, data, needs_form_wrapper, **kwargs)
        if not policy.check((("identity", "identity:get_project"),), request):
            self._columns['name'] = tables.Column('name', verbose_name=_('Name'))
        super(TenantsTable, self).__init__(request, data, needs_form_wrapper, **kwargs)
